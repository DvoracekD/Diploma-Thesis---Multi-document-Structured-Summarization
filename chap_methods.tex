...
\medskip  \clabel[pipeline-diagram]{Extraction pipeline diagram}
\picw=14cm \cinspic figs/methods/extraction-pipeline.pdf
\caption/f An overview diagram of the individual phases of the extraction pipeline process. The individual subprocesses are described in detail in this chapter. Individual blocks represent individual sub-processes. The arrows represent data passing (output to input).
\medskip
...
\picw=14cm \cinspic figs/embeded-article.png
...
\begitems
  ...
  * These articles highlight the severe impacts of global warming, evidenced by rapidly melting glaciers from Everest to the Alps, and the Great Barrier Reef losing half its corals due to rising sea temperatures. These changes, alongside record global heat like the hottest July in 142 years, are attributed to human-caused climate change.
  * The dataset consists of five randomly selected articles out of a total of 90, from the period between April 23, 2017, and May 14, 2022.\smallskip
  * \table{ccccc}{
    \crl
    \hfil People & \hfil Organizations & Locations & Events & Event summaries \crl \tskip4pt
    4 & 10 & 29 & 71 & 5 \crl
...
The data set was then divided into validation and test splits. The validation split (the global warming case) can be used for iterations of the solutions. The remainder of the dataset, that is, the other five cases, can be used to test the final solutions.

A script was created to access the dataset. The script "eval.eval_dataset.py" contains definitions of the names of media cases and paths to the source and ground truth data within the repository. The script also includes preprocessing of the data. The return values are a list of case names, a list of source articles with the properties "id" and "ftext", and also a list of ground-truth structures. The script also offers dataset splitting capability.
...
\medskip  \clabel[evaluation-diagram]{Evaluation pipeline diagram}
\picw=14cm \cinspic figs/methods/evaluation-pipeline-2.pdf
\caption/f Process diagram of the evaluation pipeline. The input to the evaluations is the predicted structure and the ground truth structure. The paths leading from them represent the transfer of these two data objects. These serve as input to all subsequent computational blocks. Independently, the calculation of individual detection metrics, depicted in purple, occurs. The outputs of these processes indicate the mapping between local IDs within the prediction and the IDs of ground truth entities and events. These mappings serve as inputs for the calculation of metrics for assigning events to articles and entities to events. The outputs of all blocks are evaluation metrics, which also represent the output of the entire process.
\medskip
...
\caption/t The table presents two event descriptions and their cosine similarity score based on embeddings from the "paraphrase-multilingual-MiniLM-L12-v2" model~\cite[multilingual-sentence-bert]. Demonstrates that semantically equivalent strings produce high similarity scores, whereas texts with less semantic overlap result in notably lower similarity values.
\endinsert
